...
char Getchar();         //清空stdin(输入缓存)
//链表操作:
void GetSchoolNode(Node** pHead, Node** pEnd);    //输入获得学校信息 
void GetProgramNode(PNode** pHead, PNode** pEnd);    //输入获得项目信息 包括选择前多少名可以获奖 

void PrintMenu();       //打印主菜单
//学校 项目 信息初始化:
void IntiInfo(Node** pHead, Node** pEnd, PNode ** ppHead, PNode ** ppEnd);  //获得学校和项目个数,为输入信息做准备 
//数字范围检验:
int CheckNum(int* num, int max, int min);   //检查在菜单输入的数字选项是否合法 
//初始化项目成绩信息:
...
//排序输出成绩:
void ManGradeSort(Node** pHead);   //男团总分排序 
void WomanGradeSort(Node** pHead);   //女团总分排序
void SchoolGradeSort(Node** pHead);   //学校总分排序
...
void Fun_7(PNode** ppHead, PNode** ppEnd, Node** pHead, Node** pEnd);     //将之前储存的项目信息读取出来 
void Fun_8(Node* pHead, PNode* ppHead);    //将项目信息生成txt文件保存在电脑内存中 
